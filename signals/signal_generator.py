...
流程：
  1. 大盘过滤（risk_on=False 直接返回空）
  2. 合并 EP + VCP 信号，去重（同只股票保留分数高的）
  3. 逐只调用 claude-sonnet-4-6 分析，返回 BUY / WATCH / SKIP
  4. 只保留 BUY，按 confidence 降序
"""
...
def _merge_signals(ep_signals: list, vcp_signals: list) -> list:
    """
    合并 EP 和 VCP 信号，同一股票只保留得分更高的那条。
    EP 使用 ep_score，VCP 使用 vcp_score，统一映射到 signal_score 字段。
    """
...
def _build_prompt(stock: dict, market_env: dict) -> str:
    ...
    score_label  = "EP评分" if signal_type == "EP" else "VCP评分"
    score_val    = stock.get("ep_score") if signal_type == "EP" else stock.get("vcp_score")
...
def generate(ep_signals: list, vcp_signals: list, market_env: dict) -> list:
    """
    综合 EP + VCP 信号，调用 Claude 分析，返回最终 BUY 信号列表。
    ...
    Returns:
        action="BUY" 的信号列表，按 confidence 降序，每条附带 Claude 分析
    """
    # ── 大盘过滤 ──────────────────────────────────────────────────────────────
    if not market_env.get("risk_on", True):
        reason = market_env.get("reason", "大盘环境不佳")
        print(f"[signal_generator] 大盘风险偏好关闭，跳过所有信号")
        print(f"  原因: {reason}")
        return []
    ...
    print(f"  EP信号: {len(ep_signals)}  VCP信号: {len(vcp_signals)}  合并后: {total} 只")
    print(f"  模型: {MODEL}")
